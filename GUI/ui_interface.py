...
import pandas as pd
from g import data,sample,select,db_update,db_del,db_atted,db_sub_atted,db_date_atted,db_updt_attd,rollno
...
"}")
        ...
        self.Excel_1.clicked.connect(lambda:self.excel(self.Excel_1))
        self.Excel_2.clicked.connect(lambda:self.excel(self.Excel_2))
        self.Excel_3.clicked.connect(lambda:self.excel(self.Excel_3))
        self.Excel_4.clicked.connect(lambda:self.excel(self.Excel_4))
        self.Excel_5.clicked.connect(lambda:self.excel(self.Excel_5))
        ...
        sub=["cs01","cs02","cs03","cs04","cs05"]
        attd=["present","absent"]
        g=['Select Gender','Male','Female','Others']
        self.comboBox.addItems(g)
        self.sub_menu_2.addItems(lang)
        self.sub_menu_3.addItems(theme)
        self.sub_menu_4.addItems(bg)
        self.sub_menu.addItems(sub)
        self.attd_status_menu.addItems(attd)
        self.tt()
        self.cls_atted()
        self.sub_atted()
        self.date_atted()
        self.calendar.selectionChanged.connect(self.date_atted)
        self.date_attd.setHorizontalHeaderLabels(['Student Name','Student ID','Sub 1','Sub 2','Sub 3','Sub 4','Sub 5'])
        self.date_attd.setColumnWidth(0,160)
        self.date_attd.setColumnWidth(1,100)
        self.sub_attd.setHorizontalHeaderLabels(['Student Name','Student ID','Sub 1','Sub 2','Sub 3','Sub 4','Sub 5'])
        self.sub_attd.setColumnWidth(0,150)
        self.sub_attd.setColumnWidth(1,100)
        self.cls_attd.setHorizontalHeaderLabels(['Student Name','Student ID','Total Hrs','Hrs Present','Hrs Absent','Percentage'])
        self.cls_attd.setColumnWidth(0,200)
        self.cls_attd.setColumnWidth(1,150)
        self.cls_attd.setColumnWidth(2,150)
        self.cls_attd.setColumnWidth(3,150)
        self.cls_attd.setColumnWidth(4,150)
        self.cls_attd.setColumnWidth(5,150)
        self.tableWidget.setHorizontalHeaderLabels(['Student Name','Student ID','Phone Number','Gender'])
        ...
        self.iv.setHidden(True)
        self.register_btn.clicked.connect(self.register)
        self.Display.clicked.connect(self.db)
        ...
        self.Update_attd.clicked.connect(self.update_attd) 
    ...
    def excel(self,b):
        if(b.objectName()=="Excel_1"):
            ch=[]
            for j in range(self.cls_attd.model().columnCount()):
                ch.append(self.cls_attd.horizontalHeaderItem(j).text())
            df=pd.DataFrame(columns=ch)

            for r in range(self.cls_attd.rowCount()):
                for c in range(self.cls_attd.columnCount()):
                    df.at[r,ch[c]]=self.cls_attd.item(r,c).text()
            df.to_excel("Class Attendance.xlsx",index=False)
        
        if(b.objectName()=="Excel_2"):
            ch=[]
            for j in range(self.sub_attd.model().columnCount()):
                ch.append(self.sub_attd.horizontalHeaderItem(j).text())
            df=pd.DataFrame(columns=ch)

            for r in range(self.sub_attd.rowCount()):
                for c in range(self.sub_attd.columnCount()):
                    df.at[r,ch[c]]=self.sub_attd.item(r,c).text()
            df.to_excel("Subject-Wise Attendance.xlsx",index=False)

        if(b.objectName()=="Excel_3"):
            ch=[]
            for j in range(self.date_attd.model().columnCount()):
                ch.append(self.date_attd.horizontalHeaderItem(j).text())
            df=pd.DataFrame(columns=ch)

            for r in range(self.date_attd.rowCount()):
                for c in range(self.date_attd.columnCount()):
                    df.at[r,ch[c]]=self.date_attd.item(r,c).text()
            d=self.calendar.selectedDate()
            d=str(d.toPython())
            fn="Attendance on "+d+".xlsx"
            df.to_excel(fn,index=False)

        if(b.objectName()=="Excel_4"):
            ch=[]
            for j in range(self.tableWidget.model().columnCount()):
                ch.append(self.tableWidget.horizontalHeaderItem(j).text())
            df=pd.DataFrame(columns=ch)

            for r in range(self.tableWidget.rowCount()):
                for c in range(self.tableWidget.columnCount()):
                    df.at[r,ch[c]]=self.tableWidget.item(r,c).text()
            df.to_excel("Student Details.xlsx",index=False)

        if(b.objectName()=="Excel_5"):
            ch=[]
            for j in range(self.tableWidget_2.model().columnCount()):
                ch.append(self.tableWidget_2.horizontalHeaderItem(j).text())
            df=pd.DataFrame(columns=ch)

            for r in range(self.tableWidget_2.rowCount()):
                for c in range(self.tableWidget_2.columnCount()):
                    df.at[r,ch[c]]=self.tableWidget_2.item(r,c).text()
            df.to_excel("tt.xlsx",index=False)


    def update_attd(self):
        id_=self.id_4.text()
        sub=self.sub_menu.currentText()
        attd_st=self.attd_status_menu.currentText()
        d=self.dateEdit.date()
        if(id_.isdigit()!=True):
            self.label_23.setText("Enter a valid Student Id")
            ...
            if(rollno(int(id_))==0):
                ...
                self.label_23.setText("Student Attendance Updated")
                db_updt_attd(id_,sub,attd_st,d.toPython())    
                self.label_23.setHidden(False)
                self.id_4.clear()
    ...
    def date_atted(self):
        ...
        attd=db_date_atted(d)
        self.date_attd.setRowCount(0)
        for row_no,row_data in enumerate(attd):
            self.date_attd.insertRow(row_no)
            for col_no,data in enumerate(row_data):
                self.date_attd.setItem(row_no,col_no,PySide2.QtWidgets.QTableWidgetItem(str(data)))
    
    def sub_atted(self):
        attd=db_sub_atted()
        self.sub_attd.setRowCount(0)
        for row_no,row_data in enumerate(attd):
            self.sub_attd.insertRow(row_no)
            for col_no,data in enumerate(row_data):
                self.sub_attd.setItem(row_no,col_no,PySide2.QtWidgets.QTableWidgetItem(str(data)))
    
    def cls_atted(self):
        attd=db_atted()
        self.cls_attd.setRowCount(0)
        for row_no,row_data in enumerate(attd):
            self.cls_attd.insertRow(row_no)
            for col_no,data in enumerate(row_data):
                self.cls_attd.setItem(row_no,col_no,PySide2.QtWidgets.QTableWidgetItem(str(data)))
    ...
    def tt(self):
        ...
        self.tableWidget_2.setRowCount(sheet.max_row-1)
        self.tableWidget_2.setHorizontalHeaderLabels(list_values[0])
        row_index=0
        for val in list_values[1:]:
            col_index=0
            for value in val:
                self.tableWidget_2.setItem(row_index,col_index,PySide2.QtWidgets.QTableWidgetItem(str(value)))
                col_index+=1
            row_index+=1

    def update(self):
        id_=self.id_2.text()
        phno=self.New_phno.text()
        if(id_.isdigit()!=True):
            self.label_15.setText("Enter a valid Student Id")
            self.label_15.setHidden(False)
            self.id_2.clear()

        elif(len(phno)!=10 or phno.isdigit()!=True):
            ...
            if(rollno(int(id_))==0):
                ...
                self.label_15.setText("Student Details Updated")
                db_update(int(id_),int(phno))
                self.label_15.setHidden(False)
                self.id_2.clear()
                self.New_phno.clear()
                self.db()
    ...
    def dele(self):
        id_=self.id_3.text()
        if(id_.isdigit()!=True):
            self.label_17.setText("Enter a valid Student Id")
            ...
            if(rollno(int(id_))==0):
                ...
                db_del(int(id_))
                self.label_17.setText("Student Details Deleted")
                self.label_17.setHidden(False)
                self.id_3.clear()
                self.db()
    ...
    def register(self):
        self.label_11.setText("Collecting Sample Photos")
        
        n=self.name.text()
        id_=self.id.text()
        ge=self.comboBox.currentText()
        phno=self.pno.text()
        password_=self.password.text()
        dob=self.dateEdit_2.date()
        
        if(n.isalpha()!=True):
            self.iv.setText("Enter a valid Name")
            self.iv.setHidden(False)            
            self.name.clear()

        elif(id_.isdigit()!=True):
            self.iv.setText("Enter a valid Student Id")
            self.iv.setHidden(False)
            self.id.clear()

        elif(ge=="Select Gender"):
            self.iv.setText("Select a Gender")
            self.iv.setHidden(False)
        
        elif(len(phno)!=10 or phno.isdigit()!=True):
            self.iv.setText("Enter a valid Phone Number")
            self.iv.setHidden(False)
            self.pno.clear() 
        

        elif(len(password_)<5):
            self.iv.setText("Enter a Password of length 5 or more")
            self.iv.setHidden(False)
            self.password.clear()
            ...
            dir='Samples/'+n
            sample(n,dir,5)
            data(n,int(id_),int(phno),ge,password_,dob.toPython())
            self.label_11.setText(n+" Registered to System Database")
            self.name.clear()
            self.id.clear()
    ...
    def db(self):
        r=select()
        self.tableWidget.setRowCount(0)
        for row_no,row_data in enumerate(r):
            self.tableWidget.insertRow(row_no)
            for col_no,data in enumerate(row_data):
                self.tableWidget.setItem(row_no,col_no,PySide2.QtWidgets.QTableWidgetItem(str(data)))
